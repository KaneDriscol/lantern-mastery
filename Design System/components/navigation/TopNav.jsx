...
const LanternMark = ({ color = 'var(--primary)' }) => (
...
export function TopNav({ activeLink, onNavigate, onSignIn, onLearnNow }) {
  return (
    <nav style={{ height: 64, background: 'var(--canvas)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 32px', borderBottom: '1px solid var(--hairline)' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
        <LanternMark />
        <span style={{ fontFamily: 'var(--font-serif-display)', fontSize: 20, color: 'var(--ink)' }}>Lantern</span>
      </div>
      <div style={{ display: 'flex', gap: 24 }}>
        {links.map(l => (
          <a key={l} onClick={() => onNavigate && onNavigate(l)} style={{
            cursor: 'pointer', textDecoration: 'none', fontFamily: 'var(--font-sans)', fontSize: 'var(--nav-link-size)',
            fontWeight: 'var(--nav-link-weight)', color: l === activeLink ? 'var(--ink)' : 'var(--muted)',
          }}>{l}</a>
        ))}
      </div>
      <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
        <TextLink href="#" onClick={onSignIn}>Sign in</TextLink>
        <Button variant="primary" onClick={onLearnNow}>Learn Now</Button>
      </div>
    </nav>
  );
}
